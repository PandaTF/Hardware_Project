	...
	D3DXMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, "../DirectX/data/fontdata.txt", L"../DirectX/data/font.dds");
	if (!result)
	{
	...
	result = UpdateSentence(m_sentence1, "sentence1", 0, 0, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "sentence2", 0, 20, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	result = InitializeSentence(&m_sentence3, 40, device);
	if (!result)
	{
		return false;
	}

	result = UpdateSentence(m_sentence3, "sentence3", 0, 40, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	result = UpdateSentence(m_sentence4, "sentence4", 0, 60, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetRenderCount(int renderCount, ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateSentence(m_sentence4, renderCountString, 20, 80, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
